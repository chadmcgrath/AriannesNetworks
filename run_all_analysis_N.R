# Master script to run all analysis steps for Neuroticism (N) dimension
# This script runs P0, P1, P2, and P3 in sequence to generate all results

cat("========================================\n")
cat("MASTER ANALYSIS SCRIPT - NEUROTICISM (N)\n")
cat("========================================\n\n")
...
# ========================================
cat("STEP 1: Running data pre-processing (P1)...\n")
p1_start <- Sys.time()
source("r_scripts/NEO & IPIP - P1 - data pre-processing & whole-sample_N.R")
p1_end <- Sys.time()
p1_duration <- as.numeric(difftime(p1_end, p1_start, units = "mins"))
cat("  P1 completed in", round(p1_duration, 2), "minutes\n")
cat("  Output: NEO & IPIP - P1_nSim50_data_N.RData\n\n")

# ========================================
# STEP 2: Resampling (P2)
# ========================================
cat("STEP 2: Running resampling (P2)...\n")
cat("  Loading P2_A file first to get random.seeds (original workflow)...\n")
load("data/NEO & IPIP - P2_nSim50_data_A.RData")  # Load P2_A to get random.seeds
if (!exists("random.seeds")) {
  stop("ERROR: random.seeds not found in P2_A file. P2_A must be run first.")
}
...
cat("  Output: NEO & IPIP - P2_nSim50_data_N.RData\n\n")

# ========================================
# STEP 3: Network Comparison & Analysis (P3)
# ========================================
cat("STEP 3: Running network comparison & analysis (P3)...\n")
cat("  WARNING: This step will take 30-60 minutes (network analysis is computationally intensive)\n")
p3_start <- Sys.time()
source("r_scripts/NEO & IPIP - P3 - netcompare & analysis_N.R")
p3_end <- Sys.time()
p3_duration <- as.numeric(difftime(p3_end, p3_start, units = "mins"))
cat("  P3 completed in", round(p3_duration, 2), "minutes\n")
cat("  Output: NEO & IPIP - P3_nSim50_data_all_N.RData\n")
cat("  Output: NEO & IPIP - P3_nSim50_results_all_N.RData\n\n")
...
cat("  2. NEO & IPIP - P1_nSim50_data_N.RData\n")
cat("  3. NEO & IPIP - P2_nSim50_data_N.RData\n")
cat("  4. NEO & IPIP - P3_nSim50_data_all_N.RData\n")
cat("  5. NEO & IPIP - P3_nSim50_results_all_N.RData\n\n")
